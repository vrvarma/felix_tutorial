...
/**
 * An implementation of {@link Balls} that moves the balls around in a
 * rectangle, and plays a sound for each ball that changes when it hits a wall.
 * 
 * @author trebor
 * 
 */
public class MovingBalls implements Balls {
    ...
    /**
     * Add a signal of a given frequency to a set of audio samples. If the total
     * signal value exceeds the percision of the samples, the signal is clipped.
     * 
     * @param frequency
     *            the frequency of the signal to add
     * @param sampleRate
     *            the number samples in a second
     * @param volume
     *            the amplitude of the signal
     * @param progress
     *            the start position the signal, initally should be zero, it
     *            will be updated by addSignal and returned, pass the returned
     *            value into subsquent calls to addSignal
     * @param samples
	 ...
	 * 
	 * @param width
	 *            width of ball bounding box
	 * @param height
	 *            height of ball bounding box
	 */
	...
	public Ball(int width, int height) {
	    ...
	    setLocation((mWidth - 2 * mRadius) / 2, (mHeight - 2 * mRadius) / 2);
	 ...
	 * 
	 * @param elapsedTime
	 *            the time which has elapsed since the last update
	 */
	 ...
	 * 
	 * @param g
	 *            the graphics area to paint onto
	 */
