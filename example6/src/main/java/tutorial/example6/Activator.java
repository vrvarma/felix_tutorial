...
public class Activator implements BundleActivator, ServiceListener {
	...
	private List m_refList = new ArrayList();
	// Maps service references to service objects.
	private Map m_refToObjMap = new HashMap();
	...
	public void serviceChanged(ServiceEvent event) {
		synchronized (m_refList) {
			...
			if (event.getType() == ServiceEvent.REGISTERED) {
				// Get the service object.
				Object service = m_context.getService(event
						.getServiceReference());
			...
			synchronized (m_refList) {
				...
				while (st.hasMoreTokens()) {
					...
					for (int i = 0; (!correct) && (i < m_refList.size()); i++) {
						DictionaryService dictionary = (DictionaryService) m_refToObjMap
								.get(m_refList.get(i));
